...
// Same trigger shape as Dropdown, but the popup leads with an embedded
// TextField filtering the option list in real time; the filter clears on close.

// Keyboard: Tab focuses the trigger, Enter/Space opens with the search
// focused; Down from the search jumps to the first match, Up returns to
// the search from it; Enter selects, Esc closes and clears the filter.
Item {
  id: root
  ...
  // Lets a parent panel know when the dropdown owns keys (search field +
  // result list active) and suspend its keyCatcher so typing doesn't drive it.
