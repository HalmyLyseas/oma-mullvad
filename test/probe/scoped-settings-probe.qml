...
ShellRoot {
  ...
  function advance() {
    ...
    if (phase === 1) {
      ...
      results.ownService = facade.serviceFor(pluginId) === service
      results.foreignServiceRejected = facade.serviceFor("foreign.plugin") === null
      results.scopedAppLibraryAbsent = facade.appLibrary === null
    ...
    if (phase === 31) {
      ...
      var staleSettings = clone(hostEntry)
      staleSettings.refreshIntervalSec = 30
      staleSettings.siblingValue = "stale"
      stalePanel.settings = staleSettings
      stalePanel.recordRecent({ countryCode: "dk", cityCode: "cph", country: "Denmark", city: "Copenhagen" })
      phase = 4
      settle.restart()
      return
    }
    ...
    if (phase === 4) {
      if (!diskEntry || (diskEntry.recentLocations || []).length !== 3 || diskEntry.recentLocations[0].cityCode !== "cph") { settle.restart(); return }
    ...
    if (phase === 61) {
      ...
      results.recoveredPanelState = panel.favoriteLocations.length === 0 && panel.recentLocations.length === 0
      panel.recordRecent({ countryCode: "se", cityCode: "got", country: "Sweden", city: "Gothenburg" })
      phase = 7
      settle.restart()
      return
    }
    ...
    if (phase === 7) {
      if (!diskEntry || (diskEntry.recentLocations || []).length !== 1) { settle.restart(); return }
      results.recreatedSaved = diskEntry.recentLocations[0].cityCode === "got"
      results.recreatedSiblingPreserved = diskEntry.siblingValue === "recreated"
        && diskEntry.pluginExtra === "recovered-plugin-field"
      results.finalRecents = diskEntry.recentLocations.length
  ...
  Timer { interval: 15000; running: true; onTriggered: root.finish("overall timeout at phase " + root.phase) }
}
